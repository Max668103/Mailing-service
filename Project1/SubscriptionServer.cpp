...
void BroadcastToClients(const std::string& message, const std::string& subscriptionType) {
    ...
    for (auto it = clientss.begin(); it != clientss.end(); ) {
        SOCKET clientSocket = it->first;
        const auto& subscriptions = it->second;

        // Перевіряємо, чи клієнт підписаний на вказаний тип
        if (subscriptions.count(subscriptionType)) {
            int result = send(clientSocket, message.c_str(), static_cast<int>(message.size()), 0);

            // Якщо виникла помилка, закриваємо сокет і видаляємо клієнта
            if (result == SOCKET_ERROR) {
                closesocket(clientSocket);
                it = clientss.erase(it);
                continue;
            }
        }
        ++it;
